...
class MViT(nn.Module):
    ...
    def forward(self, x, bboxes=None, return_attn=False):
        ...
        # print(T, H, W,self.T, self.H, self.W)
        ...
        if self.cls_embed_on:
            ...
            # print("before embedding", cls_tokens.shape, x.shape)
        ...
        if self.enable_rev:
            ...
            for id, blk in enumerate(self.blocks):
                # print("in",x.shape, thw)
                x, thw = blk(x, thw)
                t, h, w = thw
                l_x = rearrange(x, 'b (t h w) d -> b d t h w',t=t, h=h, w=w)
                layers[id] = l_x
            x_112 = layers[0]
            x_56 = layers[2]
            x_28 = layers[13]
            x_14 = layers[15]
            # print(x_112.shape, x_56.shape, x_28.shape, x_14.shape)
            if self.enable_detection:
